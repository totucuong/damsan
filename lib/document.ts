...
const ImageAnalysisSchema = z.object({
    description: z.string().describe('A detailed description of what is shown in the image'),
    objects: z.array(z.string()).describe('List of objects or items visible in the image'),
    text: z.string().nullable().optional().describe('Any text content found in the image'),
    ...
    location_type: z.string().nullable().optional().describe('Type of location if identifiable (indoor, outdoor, etc.)'),
})
...
export async function analyzeImageFromStorage(
    ...
    try {
        ...
        if (!user || user.id !== userId) {
            throw new Error('User must be authenticated and match the provided userId')
        }
